...
The layers are listed in the following table.  The base layer must be first, with higher layers later.
...
#+NAME: alternatives-body
#+BEGIN_SRC python :var layouts=layout-list :wrap "src C :noweb yes"
  ...
  def mk_layer_taphold(layer, layout, flip):
    ...
    if layer == 'base':
      ...
      args['current_layer_name'] = current_layer_name_dict[layer]
  ...
  def mk_layer_half(lr, layer, flip, variant):
    name = f'MIRYOKU_ALTERNATIVES_{layer.upper()}'
    args = {}

    args['current_layer_name'] = current_layer_name_dict[layer]
    args['opposite_layer_name'] = opposite_layer_name_dict[layer]
  ...
  for layer in iterable_layers:
    ...
    elif layer == 'base' or layer == 'tap':
      for flip in [True, False]:
        for layout in layouts:
          results += mk_layer_taphold(layer, layout, flip)
      results += '\n'

    elif lr_dict[layer] == 'r':
