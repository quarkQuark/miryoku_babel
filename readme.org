...
3. depending on the target format, sets
   - the ~basic_prefix~ and ~basic_suffix~ for wrapping most keycodes.
...
################################################################################
# Default column width when aligning lists.

width = 19

################################################################################
# Variables from Org tables.
...
################################################################################
# Variables that depend on the target format.
...
def get_shifted(code):
  ...
  if code in shifted_symbol_names_dict:
    shifted_code = shifted_symbol_names_dict[code]
    return get_translated(shifted_code)
  return None
