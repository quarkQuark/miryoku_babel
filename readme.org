#+PROPERTY: header-args:python :session :tangle no :results verbatim
...
The layer tables contain symbols as they should be recieved by the computer, but the actual keycodes sent are different.
The appropriate mapping is governed by this table.
...
Some of the codes above are aliases for shifted variants of other codes.
These symbols don't directly have their own keycodes; therefore the following table is used to translate between shifted and unshifted keys.
...
To initialise this environment, this block
...
4. defines the following helper functions:
   ...
   - ~convert_symbol~ to convert the table symbols to codes
   - ~get_translated~ to translate keycodes to the target format
   - ~get_shifted~ to return the alias keycode for a shifted key
   - ~trim_shifted~ *undocumented*.

#+NAME: table-layer-init
#+BEGIN_SRC python :var symbol_names_table=symbol-names :var shifted_names_table=shifted-names :var nonkc_table=nonkc :var nonkp_table=nonkp :var nonquote_table=nonquote :var nonkcdot_table=nonkcdot :var keycode_translation_table=keycode-translation :var layers_table=layers :var mods_table=mods :var target="svg"
import os
...
layers_dict = {layer: name for layer, name in layers_table}
...
def mod_tap(mod, code, shifted_code):
  """
  Return a keycode that sends mod when held and code (or shifted_code) when
  tapped.
  """
  match target:
    case 'qmk':
      code = wrap_basic(code)
      return f'{mod}_T({code})'
    case 'zmk':
...
def layer_tap(layer, code, shifted_code):
  """
  Return a keycode that activates layer when held and sends code (or
  shifted_code) when tapped.
  """
  match target:
    case 'qmk':
      code = wrap_basic(code)
      return f'LT({layer},{code})'
    case 'zmk':
...
def format_code(hold, code, shifted_code):
  """
  Return a keycode formatted such that it does the hold action (if recognised)
  when held and sends code (or shifted_code) when tapped.
  """
  if hold in mods_dict:
    mod = str(get_translated(hold))
    return mod_tap(mod, code, shifted_code)
  elif hold in layers_dict:
    layer = str(hold)
    return layer_tap(layer, code, shifted_code)
...
for tap_row, hold_row in zip(alphas_table + thumbs_table, hold_table):
  for tap, hold in zip(tap_row, hold_row):
    converted_tap = convert_symbol(tap)
    code = get_translated(converted_tap)
    if code == '':
      code = 'U_NU'
    shifted_code = get_shifted(converted_tap)
    wrapped_code = format_code(hold, code, shifted_code)
    results += (wrapped_code + ',').ljust(width)
  results += '\\\n'
...
#+BEGIN_SRC python :var hold_table=hold :var mode="r" :var half_table=media-r :var current_layer_name="U_MOUSE" :var opposite_layer_name="U_SYM" :var shift="false"
def layer_half(code):
...
def hold_half(hold):
  ...
  if hold == current_layer_name and target == 'svg':
    return 'U_HELD(U_NA)'
  elif hold in layers_dict or hold == '':
    return 'U_NA'
  elif hold == 'LOCK_CUR':
...
#+BEGIN_SRC python :var alphas_table=button :var thumbs_table=button-thumbs :var fill="" :var hold_table=hold :var current_layer_name="U_BUTTON"
results = ''
...
#+BEGIN_SRC python :session 'none' :var layers_table=layers
width = 8
...
#+BEGIN_SRC python :session 'none' :var layers_table=layers :var target="svg"
width = 9
results = ''
i = 0
for layer, name in layers_table:
