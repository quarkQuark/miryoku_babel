...
The ~:session~ header argument ensures that all Python source blocks share the sampe environment (unless disabled with ~:session 'none'~).  That is, they can use previously defined functions and variables.

#+NAME: table-layer-init
#+BEGIN_SRC python :var nonkc_table=nonkc :var nonkp_table=nonkp :var nonquote_table=nonquote :var nonkcdot_table=nonkcdot :var keycode_translation_table=keycode-translation :var target="svg" :var symbol_names_table=symbol-names :var shifted_names_table=shifted-names :var layers_table=layers :var mods_table=mods
import os
...
Some data depend on the target format.
When passed the target as a parameter, the following Python block sets these appropriately. It also returns a comment that is placed in the tangled file to record for which target format it was built.


width = 19  # default column width when aligning lists.
